...
// of truth release CI bumps, so we fail loudly if its shape ever drifts away
// from semver and breaks the auto-bump regex in .github/workflows/release.yml.
...
// TestVersionIsSemver verifies the constant is in major.minor.patch form so the
// release pipeline's auto-bump logic stays correct — anything else would break
// the regex in release.yml.
