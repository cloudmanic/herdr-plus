...
// the unchanged input — i.e. the pre-feature behavior is preserved exactly.
...
// TestControlModelGroupedSelect confirms the grouped browser is fully navigable:
...
func TestControlModelGroupedSelect(t *testing.T) {
	m := newControlModel(groupedProjects(), "/cfg/projects")
...
// TestControlModelGroupedFilterDropsHeadings confirms search is unchanged: a
// query collapses the grouped view to a single ranked list with no headings, and
// enter opens the lone match.
func TestControlModelGroupedFilterDropsHeadings(t *testing.T) {
	m := newControlModel(groupedProjects(), "/cfg/projects")
	m = step(t, m, tea.WindowSizeMsg{Width: 100, Height: 30})
